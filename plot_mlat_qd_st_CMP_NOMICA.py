...
if infile in os.listdir('.'):
    ...
    print('all ipc gain geomean: '+str(gm_ipc_gain))
    print('server ipc gain geomean: '+str(gm_sa_ipc_gain))
    ...
    iax1.margins(x=0.02)
    iax2.margins(x=0.02)
    plt.text(26.5,1.95,'gm_server',fontsize=fs_xtick,rotation=90, va='top')
    plt.text(27.5,1.95,'gm_desktop',fontsize=fs_xtick,rotation=90, va='top')
    plt.text(28.5,1.95,'gm_all',fontsize=fs_xtick,rotation=90, va='top')
    ...
    iax0.text(27,ipc_gains[27]+0.1,str(round(ipc_gains[27],1)),zorder=5, fontsize=fs_smalltext, ha='center');
    ...
    iax1.bar(X_axis-(barwidth*0.55), DDR_QD,edgecolor='black',alpha=alval, hatch='//', color='lavender', width=barwidth, zorder=4, bottom=lowload_mlats)
    ...
    iax1.bar(X_axis+(barwidth*0.55), cxl_30nsdel,edgecolor='black',alpha=alval, color='#bf77be',label='CXL Interface Delay', width=barwidth, zorder=4, bottom=lowload_mlats)
    ...
    iax2.legend(ncol=1,bbox_to_anchor=(1,0.4),fontsize=fs_legend)
    #iax3=iax2.twinx()
