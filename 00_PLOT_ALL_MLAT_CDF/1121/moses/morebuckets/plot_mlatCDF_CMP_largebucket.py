...
memlathist_DDR=[0]*1300;
memlathist_CXL=[0]*1300;
...
if(memlathist_exist):
    ...
    m_sum=0
    for i in range(1300):
        a=mlat_pdf_CXL[i]*(i*5)
        m_sum=m_sum+a
    print('CXL mean: '+str(m_sum))

    #find stdev
    a_sum=0
    for i in range(1300):
        #a=mlat_pdf_CXL[i]*(((i*5)-all_lat_avg/2) * ((i*5)-all_lat_avg/2) )
        a=mlat_pdf_CXL[i]*(((i*5)-m_sum) * ((i*5)-m_sum) )
        a_sum=a_sum+a
    stdev_CXL=math.sqrt(a_sum)
    print('CXL stdev: '+str(stdev_CXL))
...
cdf_x_axis=[5*x for x in range(1300)] #10*x in cycles, but / 2 to convert ot NS
